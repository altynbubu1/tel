...
bot = telebot.TeleBot('5614687172:AAG72aMrnz-htHd8MVtZEzqiqfXjFXbEDao')
...
def welcome(message):
    ...
    item1 = types.KeyboardButton('МАНХВА')
...
@bot.message_handler(func=lambda message: user_states.get(message.chat.id) == "start" and message.text == 'МАНХВА')
def choose_anime_source(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    item1 = types.KeyboardButton('МАНГАЛИБ')
    item2 = types.KeyboardButton('ReadManga')
    item3 = types.KeyboardButton('Что такое МАНХВА?')
    item4 = types.KeyboardButton('Назад')
    markup.add(item1, item2, item3, item4)
    bot.send_message(message.chat.id, 'Выберите через какой сайт будете смотреть МАНХВА', reply_markup=markup)
...
def handle_anime_source(message):
    if message.text == 'МАНГАЛИБ':
        bot.send_message(message.chat.id, 'Ссылка на МАНХВА: https://mangalib.me/')
    elif message.text == 'ReadManga':
        bot.send_message(message.chat.id, 'Ссылка на  ReadManga: https://readmanga.live/')
    elif message.text == 'Что такое МАНХВА?':
        bot.send_message(message.chat.id, 'МАНХВА - корейские комиксы.')
    elif message.text == 'Назад':
        # Вернуть пользователя в главное меню
        welcome(message)
    else:
